...
public class DbMigrationModule {
    ...
    public DbMigrationModule(Connection connection) {
        this.connection = connection;
    }

    @Provides
    public Jdbi providesJdbi() {
        // Customised connection factory that reuses a single connection without closing it
        // This is since the module does not own the connection resource.
        return Jdbi.create(() -> new NonCloseableConnectionWrapper(connection));
    }

    private static class NonCloseableConnectionWrapper implements Connection {

        private final Connection innerConnection;

        private NonCloseableConnectionWrapper(Connection connection) {
            innerConnection = connection;
        }

        @Override
        public void close() {
            // Deliberately do nothing rather than close the connection
        }
